...
module.exports = function (RED) {
    ...
    function createShelly25(config) {
        ...
        function subscribe(channel, offset) {
            broker.subscribe(node.id + offset, shelly.prefix + "/relay/" + channel, function (msg) { setRelay(channel, msg); });
            broker.subscribe(node.id + 1 + offset, shelly.prefix + "/relay/" + channel + "/power", function (msg) { setPower(channel, msg); });
            broker.subscribe(node.id + 2 + offset, shelly.prefix + "/relay/" + channel + "/energy", function (msg) { setEnergy(channel, msg); });
            broker.subscribe(node.id + 3 + offset, shelly.prefix + "/input/" + channel, function (msg) { inputReceived(msg, channel); });
        }
        if (channel === 2) {
            subscribe(0, 0);
            subscribe(1, 1);
