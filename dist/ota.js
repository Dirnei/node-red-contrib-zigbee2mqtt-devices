...
module.exports = function (RED) {
    ...
    function otaUpdate(config) {
        ...
        function otaStatusUpdateReceived(msg) {
            switch (msg.status) {
                case "available":
                    var found = bridgeNode.getDeviceList().find(function (d) { return d.friendly_name == msg.device && d.type === "Router"; });
                    if (!found) {
        ...
        function setCurrentDevice(device) {
            if (currentDevice !== device && device !== null) {
                bridgeNode.subscribeDevice(node.id, currentDevice, function (msg) {
        ...
        function startUpdate(device) {
            ...
            var found = bridgeNode.getDeviceList().find(function (d) { return d.friendly_name == device; });
            if (!found) {
