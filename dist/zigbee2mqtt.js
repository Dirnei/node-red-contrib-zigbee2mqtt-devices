...
module.exports = function (RED) {
    ...
    function sendMessages(config) {
        ...
        node.on("input", function (msg) {
            var messages = [];
            msg.payload.devices.forEach(function (element) {
                if (msg.payload.override !== undefined) {
                    if (msg.payload.override.action) {
                        ...
                        var properties = ["state", "brightness", "temperature", "color"];
    ...
    function buttonSwitch(config) {
        ...
        var node = this;
        var inputs = {};
        var types = ["Pressed", "Hold", "Released", "Double"];
        var currentIndex = 0;
        ...
        node.on("input", function (msg) {
            ...
            if (config.dynamicOutputLabels.every(function (e) { return e.toLowerCase() != actionName; })) {
                // output not enabled
                return;
            }
